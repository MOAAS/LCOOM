...
void uart_setup_fifo() {
    uart_enable_fifo();
    uart_int_info.toSend = queue_create(20000);
    uart_int_info.received = queue_create(20000);
...
int uart_fifo_send(uint8_t data) {
    ...
    // Test if communication goes both ways TIRAR ISTO 
    //uint8_t byte;
    /*
    while (status & UART_RECEIVER_DATA) {
        uart_read_data(&byte);
