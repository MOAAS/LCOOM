 ...
 */
typedef struct {
    uint16_t xMin, yMin; ///< Position of the upper left corner
    uint16_t xMax, yMax; ///< Position of the lower right corner
    uint16_t width;      ///< Canvas width (redundant)
    uint16_t height;     ///< Canvas height (redundant)
    Layer* layer;        ///< Layer to draw the canvas on.
} Canvas;
