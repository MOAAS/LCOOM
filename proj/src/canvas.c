...
void canvas_draw_image(Bitmap* bitmap) {
    if (bitmap == NULL || canvas->layer == NULL)
        return;
    int width =  bitmap->bitmapInfoHeader.width;
    int height = bitmap->bitmapInfoHeader.height;
    uint16_t yCoord = height + canvas->yMin - 1;
    uint16_t xCoord = canvas->xMin;
    uint32_t color;
    char* img = bitmap->bitmapData;    
    for (int i = 0; i < height; i++, yCoord--) {
        xCoord = canvas->xMin;
    	for(int j = 0; j < width; j++, xCoord++) {
            memcpy(&color, img, bitmap->bytes_per_pixel);
            if (is_inside_canvas(xCoord, yCoord))
                draw_on_layer(canvas->layer, xCoord, yCoord, color);
            img += bitmap->bytes_per_pixel;
        }
        img += bitmap->padding;
    }
}
